...
export function TemplateUse() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-10">
      ...
      <h1 className="mb-6 text-2xl font-semibold text-ink-50">{template.title}</h1>

      {variables.length > 0 && (
        <div className="mb-6 flex flex-col gap-3">
          {variables.map((v) => (
            <label key={v.key} className="flex flex-col gap-1 text-sm text-ink-400">
              {v.label}
              {v.required && <span className="text-brand-400"> *</span>}
              <VariableInput variable={v} value={values[v.key]} onChange={(val) => setValue(v.key, val)} />
            </label>
          ))}
        </div>
      )}
      ...
      <div className="mb-6 rounded-lg border border-ink-700 bg-ink-750 p-4">
        <p className="mb-2 text-xs tracking-wide text-ink-500 uppercase">Mensaje final</p>
        <TemplatePreview body={template.body} variables={variables} values={values} mode="values" />
      </div>
      ...
      {!canSend && (
        <p className="mb-4 text-sm text-red-400">
          Completa los campos obligatorios ({missingRequired.map((v) => v.label).join(', ')}) para
          copiar o enviar el mensaje.
        </p>
      )}
